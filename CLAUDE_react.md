...
여러 명이 같은 저장소에서 협업하는 프로젝트면(사용자가 "팀 개발"이라고 언급하는 경우 포함)
솔로 모드와 다른 운영 방식을 적용한다 — 하네스 파일을 프로젝트 저장소에 함께 커밋해서 팀원
전원이 쓸 수 있게 하고, phase 경계 직접 커밋 대신 feature 브랜치 + PR로 전환한다. 팀 규칙이
있으면 그게 최우선이라는 원칙은 전역 `CLAUDE.md`의 "팀 프로젝트에서는 팀 규칙이 최우선" 규칙
그대로다. 구체적인 절차(하네스 공유 명령, 브랜치/PR 워크플로우, 동시 진행 주의사항)는
`~/.claude/skills/react-webapp/SKILL.md`의 "협업 방식: 솔로 vs 팀" 섹션 참고 — 이 파일에서
중복 설명하지 않는다.
...
2. 팀 규칙 존재 여부를 확인한다 — 프로젝트 로컬 `CLAUDE.md`, `CONTRIBUTING.md` 등이 있으면 이
   하네스/전역 `CLAUDE.md`의 개인 규칙보다 그걸 우선한다(전역 `CLAUDE.md` "팀 프로젝트에서는
   팀 규칙이 최우선" 규칙)
3. `react-webapp` 스킬 트리거 → Phase 1에서 프로젝트 성향 + **협업 방식(솔로/팀)** 확인
   (체크포인트 0) → 승인 시 실제 `create-next-app` 스캐폴딩 실행
4. 협업 방식이 팀이면 "팀 개발 모드" 절차(하네스 공유, feature 브랜치 생성)를 스캐폴딩 직후
   바로 적용한다
5. 스캐폴딩 완료 후 바로 이어서 요구사항 정리 → 설계 → 구현 단계로 진행(같은 세션에서 자연스럽게
   이어진다, 스캐폴딩만 하고 끝내지 않는다)
6. 팀 개발이 확실하면(사용자가 이미 "팀 개발일 것 같다"고 언급하는 등) Phase 2를 솔로 모드로
